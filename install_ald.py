...
def get_default_gateway(ip_address):
    # Преобразуем строку IP-адреса в объект IPv4Address
    ip = ipaddress.IPv4Address(ip_address)

    # Получаем сеть, к которой принадлежит IP-адрес (предполагаем маску /24)
    network = ipaddress.IPv4Network(f"{ip}/24", strict=False)

    # Возвращаем первый доступный хост в сети (обычно это шлюз по умолчанию)
    return str(network.network_address + 1)

def set_form1(host, domain, admin_password):
    return [
        ("Имя хоста:", 1, 1, host, 1, 30, 20, 0, 0,  item_help1_hostname),
        ("Имя домена:", 2, 1, domain, 2, 30, 20, 0, 0, item_help1_domain),
        ("Пароль администратора:", 3, 1, admin_password, 3,
        30, 20, 0, 1, item_help1_adminpassword),
        ("Повторите пароль:", 4, 1, admin_password, 4, 30, 20, 0, 1, item_help1_confirmpassword)
    ]

def set_form2(host, domain, ip, admin_password):
    return [
        ("Имя хоста:", 1, 1, host, 1, 30, 20, 0, 0,  item_help2_hostname),
        ("Имя домена:", 2, 1, domain, 2, 30, 20, 0, 0, item_help2_domain),
        ("Ip контроллера домена:", 3, 1, ip, 3, 30, 20, 0, 0, item_help2_ip),
        ("Пароль администратора:", 4, 1, admin_password, 4, 30, 20, 0, 1, item_help2_adminpassword),
    ]


locale.setlocale(locale.LC_ALL, '')

d = pythondialog.Dialog(dialog="dialog")
...
state_percentages = {
    ...
    '[/var/log/aldpro/mp-services]': 82,
    '[/etc/apache2/sites-enabled/default-ssl.conf]': 83,
    '[apache2_restart]': 83,
    '[/etc/aldpro/aldproctl.env]': 84,
    ...
    while True:
        ...
        if action == "1 Развертывание":
            ...
            if not re.match(netbios_regex, hostname):
                ...
                form1 = set_form1("", domain, admin_password)
                continue
            ...
            if not re.match(domain_regex, domain):
                ...
                form1 = set_form1(hostname, "", admin_password)
                continue
            ...
            if not admin_password or not confirm_password:
                ...
                form1 = set_form1(hostname, domain, "")
                continue
            ...
            try:
                ...
                #d.gauge_update(
                #    100, '2/2 : Конфигурация DNS forwarding', update_text=True)
                #run_command_without_output(
                #    f"echo {admin_password} | kinit admin@{domain.upper()}")
                #run_command_without_output(
                #    f"ipa dnsconfig-mod --forwarder=77.88.8.8")
            ...
            if not re.match(netbios_regex, hostname):
                d.msgbox(
                    "Некорректное имя хоста. Пожалуйста, введите валидное netbios имя хоста.")
                form2 = set_form2("", domain, ip, admin_password)
                continue
            ...
            if not admin_password:
                ...
                form2 = set_form2(hostname, domain, ip, "")
                continue
            ...
            default_gateway = get_default_gateway(ip_address)
            ...
            try:
                ...
                run_command_to_file(
                    f'echo "auto lo\r\niface lo inet loopback\r\n\r\nauto eth0\r\niface eth0 inet static\r\n    address {ip_address}/24\r\n    gateway {default_gateway}"', '/etc/network/interfaces.d/50-cloud-init')
            ...
            try:
                ...
                os.system('clear')  # Очистка экрана
                print(
                    'Выполнено. Дождитесь завершения задачи автоматизации на портале.')
                break
